...
#define BUFFER_SIZE 4096 // 4 kilobyte
...
void printFile(const std::string &filePath)
{
    ...
    if (!in)
        std::cout << "General IO Error: Cannot open input file.\n";
...
void copyFile(const std::string &srcFilePath, const std::string &dstFilePath)
{
    ...
    std::ifstream src(srcFilePath, std::ios::binary | std::ios::in); // or to set both bits for mode :)
    std::ofstream dst;
    size_t byteQty;
    ...
    dst.open(dstFilePath, std::ios::binary | std::ios::out);
    if (!dst)
    {
        std::cout << "General IO Error: Cannot open output file";
        src.close();
        return;
    }
